...
def droplet_diameter(time,temp=TEMPERATURE): 
...
def position(time,temp=TEMPERATURE): # function that returns x and z position as tuple
...
def concentration(time,temp=TEMPERATURE): 
    distance_tuple = position(time,temp)
    sigma = A*(X_AWAY**B)
...
def exposure_per_breath(time,temp=TEMPERATURE): 
    exposure = integrate.quad(concentration, 0, time, args=(temp,)) #keep x_away constant while integrating
    #print(exposure)
    return exposure

def total_exposure(time,temp=TEMPERATURE):
    exposure_tuple = exposure_per_breath(time,temp)
    number_of_breaths = RESPIRATORY_RATE*time
    total_dosage = exposure_tuple[0]*number_of_breaths
    print(total_dosage)
    return total_exposure

def exposure_vs_temp(time,temp=TEMPERATURE):
    exposure_array = []
    diameter_array = []
